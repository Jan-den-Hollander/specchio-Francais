...
export default function App() {
  ...
  const speakIt = async (text: string) => {
    if (!text) return;
    setIsSpeaking(true);
    setStatus('Lo specchio parla... · Le miroir parle...');
    try {
  ...
  const processHeard = async (heard: string) => {
    ...
    let currentScore = 0;
    if (lastModelMsg) {
      const sim = calculateSimilarity(lastModelMsg.fr, heard);
      if (sim > 0.7) currentScore = 2; else if (sim > 0.4) currentScore = 1;
      setScore(prev => prev + currentScore);
    }
    const userMsg: Message = { role: 'user', fr: heard, it: '', heard, score: currentScore };
  ...
  const downloadTranscript = () => {
    ...
    const text = messages
      .filter(m => m.role !== 'error')
      .map(m => `[${m.role === 'user' ? 'TU' : 'SOPHIE'}]\nFR: ${m.fr}\nIT: ${m.it || '-'}\n`)
      .join('\n---\n\n');
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([text], { type: 'text/plain' }));
    a.download = 'conversazione-francese.txt';
        ...
        <div className="relative flex items-center justify-center mb-5">
          ...
          {showFlags && (
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
              ...
              <span className="text-[0.5rem] tracking-widest uppercase text-[#3f51b5]/40">Italiano</span>
            </motion.div>
          )}

          <div className="relative w-full max-w-[200px] aspect-[3/4]">
            <div className="absolute inset-0 bg-gradient-to-br from-[#1a237e] via-[#3f51b5] to-[#0d1642] rounded-[50%_50%_46%_46%_/_28%_28%_72%_72%] p-1.5 shadow-[0_10px_30px_rgba(0,0,0,0.8)]">
              <div className="w-full h-full bg-[#080818] rounded-[47%_47%_44%_44%_/_26%_26%_74%_74%] overflow-hidden relative">
                ...
                {isSpeaking && (
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1">
                    {[0, 0.15, 0.3].map((d, i) => (
                      <div key={i} className="w-1 h-3 bg-[#8fa8e8]/80 rounded-full animate-bounce" style={{ animationDelay: `${d}s` }} />
                    ))}
                  </div>
                )}
              </div>
            ...
            <button type="button" onClick={(e) => { e.preventDefault(); toggleCam(); }}
              className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-[#080810] border border-[#3f51b5]/30 px-3 py-1.5 rounded-full text-[0.55rem] tracking-widest uppercase text-[#8fa8e8]/80 flex flex-col items-center gap-0.5 z-20 w-[130px] text-center">
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="space-y-1">
            ...
            <select value={level} onChange={(e) => setLevel(e.target.value)}
              className="w-full bg-[#3f51b5]/5 border border-[#3f51b5]/20 rounded-lg px-2 py-2 text-[0.7rem] outline-none text-[#8fa8e8]">
          ...
          <div className="space-y-1">
            <label className="text-[0.55rem] uppercase tracking-widest text-[#3f51b5]/50 ml-1 flex items-center gap-1"><MessageSquare size={8} /> Argomento · Sujet</label>
            <select value={topic} onChange={(e) => setTopic(e.target.value)}
              ...
              <option value="cultura">Cultura Francese</option>
        ...
        <div className="flex items-center justify-center gap-6 mb-2">
          ...
          <div className="flex flex-col items-center gap-1">
            <button type="button" onClick={isRecording ? stopRecording : startRecording}
              className={`w-16 h-16 rounded-full flex items-center justify-center shadow-xl ${isRecording ? 'bg-red-500/20 border-2 border-red-500 animate-pulse' : 'bg-gradient-to-br from-[#3f51b5] to-[#1a237e]'}`}>
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-[0.5rem] uppercase tracking-widest text-[#3f51b5]/60 text-center leading-tight">Salta<br/><span className="text-[#3f51b5]/40">Passer</span></span>
          </div>
        </div>
        ...
        <div className="w-full h-[35vh] min-h-[250px] bg-black/30 border border-[#3f51b5]/10 rounded-xl overflow-y-auto p-3 space-y-3 scrollbar-thin mb-4">
          ...
          {isThinking && (
            <div className="flex gap-1.5 p-2 bg-[#3f51b5]/5 border border-[#3f51b5]/10 rounded-xl rounded-bl-none w-12">
              ...
              <div className="w-1 h-1 bg-[#8fa8e8] rounded-full animate-bounce [animation-delay:0.2s]" />
              <div className="w-1 h-1 bg-[#8fa8e8] rounded-full animate-bounce [animation-delay:0.4s]" />
            </div>
          )}
          <div ref={chatEndRef} />
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={downloadTranscript}
              ...
              <span className="text-[0.45rem] opacity-60">Sauvegarder</span>
            </button>
            <button type="button" onClick={() => setShowKeyModal(true)}
              className="px-4 py-2 border border-[#3f51b5]/10 rounded-lg text-[0.6rem] text-[#3f51b5]/60 hover:text-[#8fa8e8] flex flex-col items-center gap-0.5">
              <Key size={12} />
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[#080818] border border-[#3f51b5]/30 p-6 rounded-2xl w-full max-w-xs shadow-2xl">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { saveCustomKey((document.getElementById('keyInput') as HTMLInputElement).value); }}
                  className="flex-1 py-2 bg-gradient-to-r from-[#3f51b5] to-[#1a237e] rounded-lg text-white text-xs font-bold">Salva</button>
              </div>
            </motion.div>
          </motion.div>
        )}
